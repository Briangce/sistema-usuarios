(function ($) {
  "use strict"; $("#sidebar_menu").metisMenu(); $("#admin_profile_active").metisMenu(); $('.bell_notification_clicker').on('click', function () { $('.Menu_NOtification_Wrap').toggleClass('active'); }); $(document).click(function (event) { if (!$(event.target).closest(".bell_notification_clicker ,.Menu_NOtification_Wrap").length) { $("body").find(".Menu_NOtification_Wrap").removeClass("active"); } }); $('.CHATBOX_open').on('click', function () { $('.CHAT_MESSAGE_POPUPBOX').toggleClass('active'); }); $('.MSEESAGE_CHATBOX_CLOSE').on('click', function () { $('.CHAT_MESSAGE_POPUPBOX').removeClass('active'); }); $(document).click(function (event) { if (!$(event.target).closest(".CHAT_MESSAGE_POPUPBOX, .CHATBOX_open").length) { $("body").find(".CHAT_MESSAGE_POPUPBOX").removeClass("active"); } }); $(document).ready(function () {
      var proBar = $('#bar1'); if (proBar.length) { proBar.barfiller({ barColor: '#2F90F7', duration: 3000 }); }
      var proBar = $('#bar2'); if (proBar.length) { proBar.barfiller({ barColor: '#833CDF ', duration: 3000 }); }
      var proBar = $('#bar3'); if (proBar.length) { proBar.barfiller({ barColor: '#FE80B2 ', duration: 3000 }); }
      var proBar = $('#bar4'); if (proBar.length) { proBar.barfiller({ barColor: '#2FF0F7 ', duration: 3000 }); }
  }); $(".close_icon").click(function () { $(this).parents(".hide_content").slideToggle("0"); }); var count = $('.counter'); if (count.length) { count.counterUp({ delay: 100, time: 5000 }); }
  var niceSelect = $('.nice_Select'); if (niceSelect.length) { niceSelect.niceSelect(); }; var niceSelect = $('.nice_Select2'); if (niceSelect.length) { niceSelect.niceSelect(); }; var niceSelect = $('.default_sel'); if (niceSelect.length) { niceSelect.niceSelect(); }; $(document).ready(function () {
      var date_picker = $('#start_datepicker'); if (date_picker.length) { date_picker.datepicker(); }
      var date_picker = $('#end_datepicker'); if (date_picker.length) { date_picker.datepicker(); }
  ...
  var summerNote2 = $('.lms_summernote'); if (summerNote2.length) { summerNote2.summernote({ placeholder: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.', tabsize: 2, height: 305 }); }
  ...
  var owl_1 = $('.sraf_active'); if (owl_1.length) { owl_1.owlCarousel({ loop: true, margin: 30, items: 1, autoplay: true, navText: ['<i class="fa fa-angle-left"></i>', '<i class="fa fa-angle-right"></i>'], nav: true, dots: true, autoplayHoverPause: true, autoplaySpeed: 800, responsive: { 0: { items: 2 }, 767: { items: 3 }, 992: { items: 3 }, 1200: { items: 4 }, 1600: { items: 5 } } }); }
  ...
  waitForElement("#DataTables01").then(function(elem){
    new DataTable('#DataTables01', {
      order: [[7, 'desc']],
      columnDefs: [{
        visible: false
      }],
      ...
      language: {
        ...
        lengthMenu: 'Exibir _MENU_ campanhas',
        search: `<div class="box_right d-flex lms_block">
        ...
        <div class="add_button ms-2">
          <a href="#" class="btn_1" onclick="ativarModalCriar()">Criar Campanha</a>
        </div>
      </div>`,
        ...
        zeroRecords: 'Não foram encontradas campanhas com esse nome',
      }
    });
  ...
  waitForElement("#DataTables02").then(function(){
    
    new DataTable('#DataTables02', {
      order: [[5, 'desc']],
      columnDefs: [{
        visible: false
      }],
      ...
      language: {
        info: 'Exibindo página _PAGE_ de _PAGES_',
        lengthMenu: 'Exibir _MENU_ usuarios',
        search: `<div class="box_right d-flex lms_block">
        <div class="serach_field_2">
          <div class="search_inner">
            <form active="#">
        ...
        <div class="add_button ms-2">
          <a href="#" class="btn_1" onclick="ativarModalCriarUser()">Criar usuario</a>
        </div>
      </div>`,
        ...
        infoEmpty: 'Não foram encontrados registros',
        infoFiltered: '(Filtrado de _MAX_ registros totais)',
        zeroRecords: 'Não foram encontrados usuários com esse nome',
      }
    });
